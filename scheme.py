...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            operands = procedure.apply_macro(rest, env)
            return scheme_eval(operands, env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Element Number does not match')
        while formals != nil:
            child.define(formals.first, vals.first)
            if vals.second != nil and formals.second != nil:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Argument number is inaccurate.')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        x = expressions.second.first
        env.define(target, scheme_eval(x, env))
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        lambda_procedure = do_lambda_form(Pair(target.second, expressions.second), env)
...
def do_or_form(expressions, env):
    ...
    while current:
        ...
        tested = scheme_eval(current.first, env)
        rv = tested
        if scheme_truep(tested) and rv != '#f':
            return rv
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    values, formals = nil, nil
    while bindings != nil:
        bind = bindings.first
        check_form(bind, 2, 2)
        value = scheme_eval(bind.second.first, env)
        formal = bind.first
        formals, values = Pair(formal, formals), Pair(value, values)
...
def do_define_macro(expressions, env):
    ...
    try:
        ...
        if scheme_numberp(name) or len(expressions) < 2:
            raise SchemeError ("Improperly formed macro")
        macro_proc = MacroProcedure(formals, body, env)
        env.define(name, macro_proc)
        return name
    except:
        raise SchemeError ("Improperly formed macro")
    # END Problem 21
